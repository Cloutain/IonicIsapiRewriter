...
extern void TRACE(const char * format, ...);                                               // IirfLogging.c
...
extern char * Iirf_FileTimeToLocalTimeString(FILETIME * pFiletime);                        // IirfConfig.c
extern char * Iirf_SystemTimeUtcToLocalTimeString(SYSTEMTIME * pSysTime);                  // IirfConfig.c
extern IirfServerConfig * Iirf_NewServerConfig();                                          // IirfConfig.c
extern void Iirf_ReadServerConfig(IirfServerConfig * thisConfig);                          // IirfConfig.c
extern int Iirf_FindMapItemByKey( char ** key, TextMapItem *item );                        // IirfConfig.c
extern char * UrlDecode(char * encoded);                                                   // UrlDecoder.c
extern char * UrlEncode(char * decoded);                                                   // UrlDecoder.c
extern void InitializeUrlDecoder();                                                        // UrlDecoder.c
extern DWORD IirfProxy_TryRelayEmptyBodyRequest(HTTP_FILTER_CONTEXT * pfc,
                        LPCTSTR fqUrl,
                        LPCTSTR origHost,
                        int *pContentChunks,
                        int *pContentTotalBytes);                                          // Proxy.c
extern char * Iirf_Rfc1123_DateTimeNow();                                                  // Extension.c
...
extern char * Iirf_IsapiStrdup( HTTP_FILTER_CONTEXT * pfc, const char * source);           // Utils.c
extern void Iirf_GenErrorMessage(errno_t e, char * s, DWORD sz);                           // Utils.c
extern BOOL Iirf_FileExists(const TCHAR *fileName);                                        // Utils.c
